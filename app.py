...
with tab1:
    ...
    if st.button("점수 저장 및 제출", type="primary"):
        ...
        conn.update(data=df)
        st.success(
            f"[{evaluator}] 평가자의 [{target}] 대상자에 대한 평가가 구글 시트에 저장되었습니다!"
        )
...
with tab2:
    ...
    if df.empty or len(df) == 0:
        ...
        grade_counts = summary_df["기술 평가 등급"].value_counts().reindex(
...
with tab3:
    ...
    if df.empty or len(df) == 0:
        ...
        display_df["평균 점수"] = (
            display_df[ITEMS].mean(axis=1).round(2)
        )
